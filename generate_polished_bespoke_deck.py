import os
import sys
from pptx import Presentation
from pptx.util import Inches, Pt
from pptx.enum.text import PP_ALIGN
from pptx.dml.color import RGBColor
from pptx.enum.shapes import MSO_SHAPE

def build_polished_presentation():
    prs = Presentation()
    prs.slide_width = Inches(13.333333)
    prs.slide_height = Inches(7.5)
    blank_layout = prs.slide_layouts[6]

    # Clean Modern Blue & White Palette (Minimalis Modern Profesional)
    BG_LIGHT = RGBColor(248, 250, 252)       # #F8FAFC - Clean soft slate
    BG_CARD = RGBColor(255, 255, 255)        # #FFFFFF - Pure white
    
    NAVY_PRIMARY = RGBColor(15, 36, 55)      # #0F2437 - Deep Executive Navy
    NAVY_ACCENT = RGBColor(30, 58, 138)      # #1E3A8A - Rich Royal Navy
    BLUE_ACCENT = RGBColor(37, 99, 235)      # #2563EB - Electric Blue
    CYAN_ACCENT = RGBColor(14, 165, 233)     # #0EA5E9 - Sky Blue
    
    TEXT_MAIN = RGBColor(15, 23, 42)         # #0F172A - Charcoal Dark
    TEXT_MUTED = RGBColor(71, 85, 105)       # #475569 - Medium Slate
    TEXT_LIGHT = RGBColor(148, 163, 184)     # #94A3B8 - Slate Caption
    TEXT_WHITE = RGBColor(255, 255, 255)
    
    BORDER_SUBTLE = RGBColor(226, 232, 240)  # #E2E8F0 - Subtle Modern Border
    BORDER_ACCENT = RGBColor(191, 219, 254)  # #BFDBFE - Soft Blue Border
    PILL_BG_BLUE = RGBColor(239, 246, 255)   # #EFF6FF - Soft Blue Tint

    GOLD_BG = RGBColor(254, 243, 199)        # Warm Amber Tint
    GOLD_BORDER = RGBColor(245, 158, 11)     # Amber Border
    GOLD_TEXT = RGBColor(180, 83, 9)         # Amber Text
    EMERALD_TEXT = RGBColor(5, 150, 105)     # #059669

    FONT_FAMILY = "Segoe UI"

    def set_background(slide, color=BG_LIGHT):
        bg = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333333), Inches(7.5))
        bg.fill.solid()
        bg.fill.fore_color.rgb = color
        bg.line.fill.background()
        return bg

    def add_card(slide, left, top, width, height, fill_color=BG_CARD, border_color=BORDER_SUBTLE, radius=0.025):
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, height)
        card.fill.solid()
        card.fill.fore_color.rgb = fill_color
        if border_color:
            card.line.color.rgb = border_color
            card.line.width = Pt(1)
        else:
            card.line.fill.background()
        if hasattr(card, "adjustments") and len(card.adjustments) > 0:
            card.adjustments[0] = radius
        return card

    def add_pill_tag(slide, left, top, width, height, text, bg_color=PILL_BG_BLUE, text_color=BLUE_ACCENT, font_size=9.5):
        pill = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, height)
        pill.fill.solid()
        pill.fill.fore_color.rgb = bg_color
        pill.line.color.rgb = BORDER_ACCENT
        pill.line.width = Pt(0.75)
        if hasattr(pill, "adjustments") and len(pill.adjustments) > 0:
            pill.adjustments[0] = 0.25
        tf = pill.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_right = tf.margin_top = tf.margin_bottom = Inches(0.04)
        p = tf.paragraphs[0]
        p.text = text
        p.alignment = PP_ALIGN.CENTER
        p.font.name = FONT_FAMILY
        p.font.size = Pt(font_size)
        p.font.bold = True
        p.font.color.rgb = text_color
        for r in p.runs:
            r.font.name = FONT_FAMILY
            r.font.size = Pt(font_size)
            r.font.bold = True
            r.font.color.rgb = text_color
        return pill

    def add_p(tf, text="", font_size=10, bold=False, italic=False, color=TEXT_MAIN, space_after=Pt(3), space_before=Pt(0), alignment=PP_ALIGN.LEFT):
        if len(tf.paragraphs) == 1 and tf.paragraphs[0].text == "":
            p = tf.paragraphs[0]
        else:
            p = tf.add_paragraph()
        p.text = text
        p.alignment = alignment
        p.space_after = space_after
        p.space_before = space_before
        p.font.name = FONT_FAMILY
        p.font.size = Pt(font_size)
        p.font.bold = bold
        p.font.italic = italic
        if color:
            p.font.color.rgb = color
        for r in p.runs:
            r.font.name = FONT_FAMILY
            r.font.size = Pt(font_size)
            r.font.bold = bold
            r.font.italic = italic
            if color:
                r.font.color.rgb = color
        return p

    def add_header(slide, tag_text, title_text, subtitle_text=""):
        add_pill_tag(slide, Inches(0.8), Inches(0.46), Inches(3.8), Inches(0.32), tag_text, PILL_BG_BLUE, BLUE_ACCENT, 9)
        tb = slide.shapes.add_textbox(Inches(0.8), Inches(0.84), Inches(11.7), Inches(0.55))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = tf.margin_right = tf.margin_bottom = 0
        add_p(tf, title_text, font_size=21, bold=True, color=NAVY_PRIMARY, space_after=Pt(2))

        if subtitle_text:
            tb_sub = slide.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(11.7), Inches(0.35))
            tf_sub = tb_sub.text_frame
            tf_sub.word_wrap = True
            tf_sub.margin_left = tf_sub.margin_top = tf_sub.margin_right = tf_sub.margin_bottom = 0
            add_p(tf_sub, subtitle_text, font_size=11.5, bold=False, color=TEXT_MUTED)

    # =========================================================================
    # SLIDE 1: COVER SLIDE
    # =========================================================================
    slide1 = prs.slides.add_slide(blank_layout)
    set_background(slide1, BG_LIGHT)

    top_bar = slide1.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, Inches(13.333333), Inches(0.08))
    top_bar.fill.solid()
    top_bar.fill.fore_color.rgb = BLUE_ACCENT
    top_bar.line.fill.background()

    # Left Column: Information Card
    left_hero = add_card(slide1, Inches(0.8), Inches(0.65), Inches(7.6), Inches(6.2), BG_CARD, BORDER_SUBTLE, radius=0.02)
    add_pill_tag(slide1, Inches(1.2), Inches(0.95), Inches(3.4), Inches(0.32), "PORTOFOLIO PROFESIONAL 2026", PILL_BG_BLUE, BLUE_ACCENT, 9)

    name_box = slide1.shapes.add_textbox(Inches(1.2), Inches(1.35), Inches(6.8), Inches(0.8))
    tf = name_box.text_frame
    tf.word_wrap = True
    tf.margin_left = tf.margin_top = 0
    add_p(tf, "Khairul Raihan Hidayat, S.Kom.", font_size=27, bold=True, color=NAVY_PRIMARY)

    role_box = slide1.shapes.add_textbox(Inches(1.2), Inches(2.2), Inches(6.8), Inches(0.45))
    tf_role = role_box.text_frame
    tf_role.word_wrap = True
    tf_role.margin_left = tf_role.margin_top = 0
    add_p(tf_role, "Data Analyst & Agentic AI Specialist", font_size=16.5, bold=True, color=BLUE_ACCENT)

    # Academic Distinction Badge
    honor_pill = slide1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.2), Inches(2.72), Inches(6.8), Inches(0.42))
    honor_pill.fill.solid()
    honor_pill.fill.fore_color.rgb = GOLD_BG
    honor_pill.line.color.rgb = GOLD_BORDER
    honor_pill.line.width = Pt(1)
    if hasattr(honor_pill, "adjustments") and len(honor_pill.adjustments) > 0:
        honor_pill.adjustments[0] = 0.15
    tf_h = honor_pill.text_frame
    tf_h.margin_left = Inches(0.15)
    tf_h.margin_top = Inches(0.06)
    add_p(tf_h, "★ Sarjana Sistem Informasi (Data Science) | IPK 3.88 / 4.00 (Magna Cum Laude)", font_size=11, bold=True, color=GOLD_TEXT)

    bio_box = slide1.shapes.add_textbox(Inches(1.2), Inches(3.28), Inches(6.8), Inches(1.3))
    tf_bio = bio_box.text_frame
    tf_bio.word_wrap = True
    tf_bio.margin_left = tf_bio.margin_top = 0
    add_p(tf_bio, "Lulusan terbaik Universitas Budi Luhur yang memadukan keahlian analitika data kuantitatif, pemodelan Machine Learning (NLP/Klasifikasi), visualisasi data eksekutif (Tableau & SQL), serta rekayasa alur kerja otomatisasi berbasis Google Antigravity Agentic AI.", font_size=11, color=TEXT_MUTED)

    # Contact Cards Grid (2x2)
    contacts = [
        ("EMAIL", "khairulraihan1204@gmail.com"),
        ("WHATSAPP", "+62 822-1081-3652"),
        ("LINKEDIN", "linkedin.com/in/khairulraihan"),
        ("WEB PORTOFOLIO", "khairulraihan.github.io")
    ]
    for idx, (label, val) in enumerate(contacts):
        c_left = Inches(1.2 + (idx % 2) * 3.45)
        c_top = Inches(4.8 + (idx // 2) * 0.85)
        card = add_card(slide1, c_left, c_top, Inches(3.35), Inches(0.72), BG_LIGHT, BORDER_SUBTLE, radius=0.06)
        tb = slide1.shapes.add_textbox(c_left + Inches(0.15), c_top + Inches(0.08), Inches(3.1), Inches(0.55))
        tf_c = tb.text_frame
        tf_c.margin_left = tf_c.margin_top = 0
        add_p(tf_c, label, font_size=8.5, bold=True, color=BLUE_ACCENT, space_after=Pt(1))
        add_p(tf_c, val, font_size=9.5, bold=False, color=TEXT_MAIN)

    # Right Column: Visual Portrait & Key Metrics Card
    right_card = add_card(slide1, Inches(8.65), Inches(0.65), Inches(3.85), Inches(6.2), NAVY_PRIMARY, border_color=None, radius=0.02)
    
    profile_path = os.path.abspath("assets/images/profile.jpg")
    if os.path.exists(profile_path):
        slide1.shapes.add_picture(profile_path, Inches(9.35), Inches(1.0), Inches(2.45), Inches(2.45))
        frame = slide1.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(9.3), Inches(0.95), Inches(2.55), Inches(2.55))
        frame.fill.background()
        frame.line.color.rgb = CYAN_ACCENT
        frame.line.width = Pt(2)

    stats_box = slide1.shapes.add_textbox(Inches(8.9), Inches(3.65), Inches(3.4), Inches(2.9))
    tf_stat = stats_box.text_frame
    tf_stat.word_wrap = True
    tf_stat.margin_left = tf_stat.margin_top = 0
    add_p(tf_stat, "METRIK & PENCAPAIAN UTAMA", font_size=10.5, bold=True, color=CYAN_ACCENT, space_after=Pt(6))

    metrics_list = [
        ("3.88 / 4.00", "IPK Lulusan Terbaik (Magna Cum Laude)"),
        ("88.41%", "Akurasi Linear SVM Skripsi NLP"),
        ("8.950+", "Catatan Karyawan Dianalisis di Tableau"),
        ("~70%", "Efisiensi Otomasi Workflow Agentic AI")
    ]
    for val, desc in metrics_list:
        add_p(tf_stat, f"■  {val}", font_size=11, bold=True, color=TEXT_WHITE, space_after=Pt(0))
        add_p(tf_stat, desc, font_size=8.5, color=RGBColor(203, 213, 225), space_after=Pt(4))

    # =========================================================================
    # SLIDE 2: PROFIL PROFESIONAL & 3 PILAR STRATEGIS
    # =========================================================================
    slide2 = prs.slides.add_slide(blank_layout)
    set_background(slide2, BG_LIGHT)
    add_header(slide2, "EXECUTIVE PROFILE", "Profil Profesional & 3 Pilar Kompetensi Utama", "Mengintegrasikan ketajaman analitis, kecakapan business intelligence, dan orkestrasi Agentic AI.")

    summary_card = add_card(slide2, Inches(0.8), Inches(1.85), Inches(11.7), Inches(1.05), BG_CARD, BORDER_SUBTLE, radius=0.015)
    tb = slide2.shapes.add_textbox(Inches(1.05), Inches(1.95), Inches(11.2), Inches(0.85))
    tf = tb.text_frame
    tf.word_wrap = True
    tf.margin_left = tf.margin_top = 0
    add_p(tf, "Khairul Raihan Hidayat adalah Sarjana Sistem Informasi (Data Science) dari Universitas Budi Luhur dengan predikat Magna Cum Laude (IPK 3.88). Memiliki fondasi analitika data yang kuat, spesialisasi pemrosesan bahasa alami (NLP), pembuatan interactive executive dashboards menggunakan Tableau & SQL, serta perancangan sistem otomasi multi-agen Google Antigravity.", font_size=11, color=TEXT_MAIN)

    pillars = [
        {
            "tag": "PILAR 01",
            "title": "Machine Learning & NLP",
            "subtitle": "Predictive Modeling & Text Analytics",
            "points": [
                "Pemodelan klasifikasi terapan: Support Vector Machine (SVM), Naive Bayes, K-Nearest Neighbors (KNN).",
                "Pipeline text preprocessing komprehensif: Case folding, normalisasi slang/kata gaul, stopword Sastrawi.",
                "Ekstraksi fitur Sparse Matrix berdimensi tinggi via CountVectorizer & TF-IDF (11.790 vocab unik).",
                "Evaluasi ketat via Stratified Split, Confusion Matrix, Precision, Recall, dan F1-Score."
            ],
            "accent": BLUE_ACCENT
        },
        {
            "tag": "PILAR 02",
            "title": "Business Intelligence & EDA",
            "subtitle": "Interactive Dashboards & Decision Support",
            "points": [
                "Perancangan visualisasi data interaktif dan storytelling eksekutif menggunakan Tableau Desktop.",
                "Eksplorasi dataset korporat skala ribuan baris (HR workforce 8.950 records, e-commerce retail).",
                "Query data relasional kompleks dengan SQL: Aggregations, CTEs, Joins, Window Functions.",
                "Analisis metrik bisnis krusial: Retention Rate, Attrition Risk, Regional Profit Margin, dan Pola Pembelian."
            ],
            "accent": NAVY_ACCENT
        },
        {
            "tag": "PILAR 03",
            "title": "Agentic AI Automation",
            "subtitle": "Multi-Agent Systems & Process Engineering",
            "points": [
                "Perancangan arsitektur multi-agen cerdas memanfaatkan ekosistem Google Antigravity Framework.",
                "Otomasi rantai produksi konten dari kurasi transkrip podcast hingga short-form video viral 9:16.",
                "Integrasi AI vision, programmatic scripting (CapCut/FFmpeg), dan prompt engineering presisi.",
                "Mencapai ~70% efisiensi waktu turnaround produksi dan eliminasi bottleneck manual."
            ],
            "accent": CYAN_ACCENT
        }
    ]

    for idx, col in enumerate(pillars):
        c_left = Inches(0.8 + idx * 4.0)
        c_top = Inches(3.1)
        c_w = Inches(3.7)
        c_h = Inches(3.9)

        card = add_card(slide2, c_left, c_top, c_w, c_h, BG_CARD, BORDER_SUBTLE, radius=0.02)
        strip = slide2.shapes.add_shape(MSO_SHAPE.RECTANGLE, c_left, c_top, c_w, Inches(0.08))
        strip.fill.solid()
        strip.fill.fore_color.rgb = col["accent"]
        strip.line.fill.background()

        tb = slide2.shapes.add_textbox(c_left + Inches(0.25), c_top + Inches(0.2), c_w - Inches(0.5), c_h - Inches(0.35))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = 0

        add_p(tf, col["tag"], font_size=9.5, bold=True, color=col["accent"], space_after=Pt(2))
        add_p(tf, col["title"], font_size=13.5, bold=True, color=NAVY_PRIMARY, space_after=Pt(1))
        add_p(tf, col["subtitle"], font_size=9.5, italic=True, color=TEXT_MUTED, space_after=Pt(6))

        for pt in col["points"]:
            add_p(tf, f"• {pt}", font_size=9, color=TEXT_MAIN, space_after=Pt(3))

    # =========================================================================
    # SLIDE 3: KEAHLIAN TEKNIS & EKOSISTEM TOOLING
    # =========================================================================
    slide3 = prs.slides.add_slide(blank_layout)
    set_background(slide3, BG_LIGHT)
    add_header(slide3, "TECHNICAL TAXONOMY", "Keahlian Teknis & Ekosistem Tooling", "Kapabilitas teknis end-to-end dari data ingestion, preprocessing, pemodelan, hingga deployment.")

    skill_groups = [
        {
            "category": "Bahasa Pemrograman & Querying",
            "skills": [
                ("Python", "Pandas, NumPy, Scikit-Learn, SciPy, Matplotlib, Seaborn"),
                ("SQL (Structured Query)", "PostgreSQL, MySQL, Window Functions, Complex Joins, CTEs"),
                ("R Language", "Statistical computing & exploratory baseline modeling"),
                ("Data Manipulation", "Regular Expressions (Regex), JSON, CSV, API Ingestion")
            ]
        },
        {
            "category": "Data Science & Machine Learning",
            "skills": [
                ("Algoritma Klasifikasi", "Support Vector Machine (Linear SVM), Naive Bayes, KNN"),
                ("Natural Language Processing", "CountVectorizer, TF-IDF, Sastrawi Stemmer, Lexicon-based"),
                ("Evaluasi Model", "Stratified K-Fold, Confusion Matrix, F1-Score, ROC-AUC"),
                ("Hyperparameter Tuning", "GridSearchCV, RandomSearch, Pipeline Automation")
            ]
        },
        {
            "category": "Business Intelligence & Visualization",
            "skills": [
                ("Tableau Desktop & Public", "Calculated Fields, LOD Expressions, Parameter Actions, Sets"),
                ("Web Application", "Streamlit (Live interactive Machine Learning demo web apps)"),
                ("Executive Storytelling", "KPI Dashboarding, Geospatial Mapping, Scatter & Funnel Plots"),
                ("Exploratory Data Analysis", "Trend detection, outlier identification, cohort retention")
            ]
        },
        {
            "category": "Agentic AI, DevOps & Tooling",
            "skills": [
                ("Agentic AI Frameworks", "Google Antigravity, Multi-agent autonomous workflow design"),
                ("Version Control", "Git, GitHub, Branching Strategy, CI/CD automation"),
                ("Workflow Automation", "Python Automation Scripts, CapCut API batching, FFmpeg"),
                ("Development Environments", "VS Code, Jupyter Notebook, Google Colab, Windows PowerShell")
            ]
        }
    ]

    for idx, grp in enumerate(skill_groups):
        r = idx // 2
        c = idx % 2
        c_left = Inches(0.8 + c * 6.0)
        c_top = Inches(1.95 + r * 2.6)
        c_w = Inches(5.7)
        c_h = Inches(2.4)

        card = add_card(slide3, c_left, c_top, c_w, c_h, BG_CARD, BORDER_SUBTLE, radius=0.02)
        
        header_bg = slide3.shapes.add_shape(MSO_SHAPE.RECTANGLE, c_left, c_top, c_w, Inches(0.42))
        header_bg.fill.solid()
        header_bg.fill.fore_color.rgb = PILL_BG_BLUE
        header_bg.line.color.rgb = BORDER_ACCENT
        tf_h = header_bg.text_frame
        tf_h.margin_left = Inches(0.15)
        tf_h.margin_top = Inches(0.08)
        add_p(tf_h, f"❖  {grp['category']}", font_size=10.5, bold=True, color=NAVY_ACCENT)

        tb = slide3.shapes.add_textbox(c_left + Inches(0.2), c_top + Inches(0.52), c_w - Inches(0.4), c_h - Inches(0.55))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = 0

        for title, details in grp["skills"]:
            p = tf.add_paragraph() if len(tf.paragraphs) > 0 and tf.paragraphs[0].text else tf.paragraphs[0]
            p.text = f"• {title}: "
            p.font.name = FONT_FAMILY
            p.font.size = Pt(9.5)
            p.font.bold = True
            p.font.color.rgb = NAVY_PRIMARY
            p.space_after = Pt(2)
            for r_s in p.runs:
                r_s.font.name = FONT_FAMILY
                r_s.font.size = Pt(9.5)
                r_s.font.bold = True
                r_s.font.color.rgb = NAVY_PRIMARY

            r_det = p.add_run()
            r_det.text = details
            r_det.font.name = FONT_FAMILY
            r_det.font.size = Pt(9)
            r_det.font.bold = False
            r_det.font.color.rgb = TEXT_MUTED

    # =========================================================================
    # SLIDE 4: PROJECT 1 - SKRIPSI NLP (OVERVIEW & ACCURACY COMPARISON)
    # =========================================================================
    slide4 = prs.slides.add_slide(blank_layout)
    set_background(slide4, BG_LIGHT)
    add_header(slide4, "PROJECT 01: MACHINE LEARNING SKRIPSI", "Komparasi SVM, Naive Bayes & KNN: Analisis Sentimen YouTube", "Studi Kasus Respons Netizen terhadap Pengungkapan Kasus Korupsi Videografer di Sumatera Utara (Kanal Ferry Irwandi).")

    left_card = add_card(slide4, Inches(0.8), Inches(1.85), Inches(5.6), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    tb_l = slide4.shapes.add_textbox(Inches(1.05), Inches(2.0), Inches(5.1), Inches(4.85))
    tf_l = tb_l.text_frame
    tf_l.word_wrap = True
    tf_l.margin_left = tf_l.margin_top = 0

    add_p(tf_l, "RINGKASAN PENELITIAN & METODOLOGI", font_size=11, bold=True, color=BLUE_ACCENT, space_after=Pt(3))
    add_p(tf_l, "Penelitian skripsi ini membandingkan kinerja tiga algoritma Machine Learning terpopuler dalam mengklasifikasikan respons sentimen publik pada 9.269 komentar YouTube kasus korupsi. Dataset dikonversi menjadi representasi numerik menggunakan CountVectorizer menghasilkan ruang sparse matrix berdimensi 11.790 fitur unik.", font_size=9.5, color=TEXT_MAIN, space_after=Pt(6))

    add_p(tf_l, "TEMUAN UTAMA KOMPARASI ALGORITMA:", font_size=10, bold=True, color=NAVY_PRIMARY, space_after=Pt(3))

    findings = [
        ("Linear SVM (88.41% Akurasi)", "Mendominasi secara mutlak di seluruh skenario split (60:40, 70:30, 80:20). Sangat tangguh mengatasi fenomena high-dimensional sparsity."),
        ("Multinomial Naive Bayes (81.03%)", "Performa klasifikasi moderat. Cepat dalam inferensi, namun terkendala asumsi independensi bersyarat pada konteks kalimat kritik publik."),
        ("K-Nearest Neighbors (68.22%)", "Performa terendah akibat kutukan dimensi (curse of dimensionality), di mana perhitungan jarak Euclidean kehilangan diskriminasi pada 11.790 fitur.")
    ]
    for m_title, m_desc in findings:
        p_m = tf_l.add_paragraph()
        p_m.text = f"✓ {m_title}: "
        p_m.font.name = FONT_FAMILY
        p_m.font.size = Pt(9)
        p_m.font.bold = True
        p_m.font.color.rgb = BLUE_ACCENT
        p_m.space_after = Pt(2)
        for r_m in p_m.runs:
            r_m.font.name = FONT_FAMILY
            r_m.font.size = Pt(9)
            r_m.font.bold = True
            r_m.font.color.rgb = BLUE_ACCENT
        run = p_m.add_run()
        run.text = m_desc
        run.font.name = FONT_FAMILY
        run.font.size = Pt(8.5)
        run.font.bold = False
        run.font.color.rgb = TEXT_MUTED

    # Right Column: Embed Cropped Real Thesis Accuracy Comparison Chart
    right_card = add_card(slide4, Inches(6.6), Inches(1.85), Inches(5.9), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    chart_acc_path = os.path.abspath("assets/cropped_charts/chart_accuracy_comparison.png")
    if os.path.exists(chart_acc_path):
        slide4.shapes.add_picture(chart_acc_path, Inches(6.75), Inches(2.0), Inches(5.6), Inches(4.35))
        tb_cap = slide4.shapes.add_textbox(Inches(6.75), Inches(6.42), Inches(5.6), Inches(0.45))
        tf_cap = tb_cap.text_frame
        tf_cap.margin_left = tf_cap.margin_top = 0
        add_p(tf_cap, "Grafik Komparasi Resmi Sidang Skripsi (Universitas Budi Luhur, 2026): Linear SVM mencapai puncak akurasi 88.41% pada rasio split 80:20.", font_size=8.5, italic=True, color=TEXT_MUTED)

    # =========================================================================
    # SLIDE 5: PROJECT 1 - PIPELINE NLP, CONFUSION MATRIX & STREAMLIT APP
    # =========================================================================
    slide5 = prs.slides.add_slide(blank_layout)
    set_background(slide5, BG_LIGHT)
    add_header(slide5, "PROJECT 01: ARCHITECTURE & EVALUATION", "Arsitektur Pipeline NLP & Analisis Confusion Matrix", "Dari ekstraksi YouTube Data API v3, kamus slang normalisasi, hingga deploy Streamlit Web App.")

    pipeline_steps = [
        ("1. Data Ingestion", "9.269 Komentar Mentah diekstrak via YouTube Data API v3 Google Cloud."),
        ("2. Regex Cleaning", "Pembersihan URL, hashtag, angka, tanda baca & emoji via Regex."),
        ("3. Normalisasi Slang", "Mapping kata gaul/slang bahasa Indonesia ke bentuk baku terstruktur."),
        ("4. Stopwords Sastrawi", "Penghapusan kata sambung korpus Sastrawi + kamus buatan."),
        ("5. Feature Vector", "CountVectorizer sparse matrix menghasilkan 11.790 vocab unik.")
    ]

    for idx, (p_title, p_desc) in enumerate(pipeline_steps):
        p_left = Inches(0.8 + idx * 2.4)
        p_top = Inches(1.85)
        p_w = Inches(2.25)
        p_h = Inches(1.2)

        card = add_card(slide5, p_left, p_top, p_w, p_h, BG_CARD, BORDER_SUBTLE, radius=0.04)
        tb = slide5.shapes.add_textbox(p_left + Inches(0.12), p_top + Inches(0.08), p_w - Inches(0.24), p_h - Inches(0.16))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = 0
        add_p(tf, p_title, font_size=9.5, bold=True, color=BLUE_ACCENT, space_after=Pt(2))
        add_p(tf, p_desc, font_size=8, color=TEXT_MUTED)

    # Bottom Left: Embed Cropped Confusion Matrix Chart
    bot_left_card = add_card(slide5, Inches(0.8), Inches(3.2), Inches(5.9), Inches(3.8), BG_CARD, BORDER_SUBTLE, radius=0.02)
    chart_cm_path = os.path.abspath("assets/cropped_charts/chart_confusion_matrix.png")
    if os.path.exists(chart_cm_path):
        slide5.shapes.add_picture(chart_cm_path, Inches(0.95), Inches(3.3), Inches(5.6), Inches(3.55))

    # Bottom Right: Rigorous Statistical Breakdown & Streamlit Web App
    bot_right_card = add_card(slide5, Inches(7.0), Inches(3.2), Inches(5.5), Inches(3.8), BG_CARD, BORDER_SUBTLE, radius=0.02)
    tb_r = slide5.shapes.add_textbox(Inches(7.25), Inches(3.35), Inches(5.0), Inches(3.5))
    tf_r = tb_r.text_frame
    tf_r.word_wrap = True
    tf_r.margin_left = tf_r.margin_top = 0

    add_p(tf_r, "ANALISIS VALIDASI MATRIKS & DEPLOYMENT", font_size=10.5, bold=True, color=NAVY_PRIMARY, space_after=Pt(4))

    breakdown_points = [
        ("Total Data Uji Evaluasi", "1.476 Baris komentar (rasio split 80:20)"),
        ("True Positive & True Negative", "808 Sentimen Positif & 497 Sentimen Negatif terklasifikasi akurat."),
        ("Presisi Kelas Negatif (88.6%)", "Model sangat selektif dan tidak gegabah melabeli kritik masyarakat sebagai ujaran negatif tanpa konteks."),
        ("Recall Kelas Positif (92.7%)", "Model sangat sensitif dan akurat dalam menangkap narasi dukungan terhadap penegakan hukum korupsi."),
        ("Streamlit Interactive Deployment", "Model diekspor via Pickle dan diintegrasikan ke web app Streamlit untuk inferensi sentimen real-time.")
    ]

    for b_title, b_desc in breakdown_points:
        p_b = tf_r.add_paragraph()
        p_b.text = f"• {b_title}: "
        p_b.font.name = FONT_FAMILY
        p_b.font.size = Pt(9)
        p_b.font.bold = True
        p_b.font.color.rgb = BLUE_ACCENT
        p_b.space_after = Pt(2)
        for r_b in p_b.runs:
            r_b.font.name = FONT_FAMILY
            r_b.font.size = Pt(9)
            r_b.font.bold = True
            r_b.font.color.rgb = BLUE_ACCENT
        run = p_b.add_run()
        run.text = b_desc
        run.font.name = FONT_FAMILY
        run.font.size = Pt(8.5)
        run.font.bold = False
        run.font.color.rgb = TEXT_MAIN

    # =========================================================================
    # SLIDE 6: PROJECT 2 - TABLEAU HR ANALYTICS DASHBOARD
    # =========================================================================
    slide6 = prs.slides.add_slide(blank_layout)
    set_background(slide6, BG_LIGHT)
    add_header(slide6, "PROJECT 02: BUSINESS INTELLIGENCE", "Tableau HR Analytics: Workforce & Retention Dashboard", "Analisis komprehensif 8.950 catatan karyawan untuk mendeteksi tren retensi, kompensasi, dan performa.")

    left_hr_card = add_card(slide6, Inches(0.8), Inches(1.85), Inches(6.8), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    hr_img = os.path.abspath("assets/images/project-hr-analytics.png")
    if os.path.exists(hr_img):
        slide6.shapes.add_picture(hr_img, Inches(0.95), Inches(2.0), Inches(6.5), Inches(4.45))
        tb_hr_cap = slide6.shapes.add_textbox(Inches(0.95), Inches(6.52), Inches(6.5), Inches(0.38))
        tf_cap = tb_hr_cap.text_frame
        tf_cap.margin_left = tf_cap.margin_top = 0
        add_p(tf_cap, "Interactive Tableau Desktop Dashboard: Employee Retention, State Map & Performance Scatter Plot.", font_size=8.5, italic=True, color=TEXT_MUTED)

    right_hr_card = add_card(slide6, Inches(7.8), Inches(1.85), Inches(4.7), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    
    kpis = [
        ("8.950", "TOTAL EMPLOYEES", "Basis data tenaga kerja dianalisis"),
        ("89.2%", "RETENTION RATE", "Tingkat loyalitas karyawan aktif"),
        ("10.8%", "TURNOVER RATE", "Tingkat atrisi teridentifikasi")
    ]
    for idx, (val, title, sub) in enumerate(kpis):
        k_top = Inches(2.0 + idx * 0.95)
        k_card = add_card(slide6, Inches(8.0), k_top, Inches(4.3), Inches(0.82), PILL_BG_BLUE, BORDER_ACCENT, radius=0.04)
        tb_k = slide6.shapes.add_textbox(Inches(8.15), k_top + Inches(0.08), Inches(4.0), Inches(0.68))
        tf_k = tb_k.text_frame
        tf_k.margin_left = tf_k.margin_top = 0
        
        p_val = tf_k.paragraphs[0]
        p_val.text = f"{val}  "
        p_val.font.name = FONT_FAMILY
        p_val.font.size = Pt(16)
        p_val.font.bold = True
        p_val.font.color.rgb = NAVY_PRIMARY
        for r_k in p_val.runs:
            r_k.font.name = FONT_FAMILY
            r_k.font.size = Pt(16)
            r_k.font.bold = True
            r_k.font.color.rgb = NAVY_PRIMARY

        r_t = p_val.add_run()
        r_t.text = f"— {title}"
        r_t.font.name = FONT_FAMILY
        r_t.font.size = Pt(9.5)
        r_t.font.bold = True
        r_t.font.color.rgb = BLUE_ACCENT
        
        add_p(tf_k, sub, font_size=8.5, color=TEXT_MUTED)

    tb_hr_ins = slide6.shapes.add_textbox(Inches(8.0), Inches(5.0), Inches(4.3), Inches(1.9))
    tf_ins = tb_hr_ins.text_frame
    tf_ins.word_wrap = True
    tf_ins.margin_left = tf_ins.margin_top = 0

    add_p(tf_ins, "STRATEGIC HR INSIGHTS:", font_size=10, bold=True, color=NAVY_PRIMARY, space_after=Pt(3))

    hr_points = [
        "Kompensasi vs Performa: Analisis scatter plot mengidentifikasi retensi talenta berkinerja tinggi rentan terhadap stagnasi gaji.",
        "Geographic Mapping: Visualisasi sebaran kantor cabang membantu pemerataan beban kerja dan perencanaan tunjangan regional.",
        "Rekomendasi Kebijakan: Penyesuaian skema review performa semesteran untuk memitigasi turnover pada divisi berisiko tinggi."
    ]
    for pt in hr_points:
        add_p(tf_ins, f"• {pt}", font_size=8.5, color=TEXT_MAIN, space_after=Pt(3))

    # =========================================================================
    # SLIDE 7: PROJECT 3 - AGENTIC AI VIDEO & AUTOMATION PIPELINE (WITH REAL DASHBOARD)
    # =========================================================================
    slide7 = prs.slides.add_slide(blank_layout)
    set_background(slide7, BG_LIGHT)
    add_header(slide7, "PROJECT 03: AGENTIC AI AUTOMATION", "Double Coverage Auto-Clipper: Multi-Agent Video Pipeline", "Sistem otomasi alur kerja cerdas untuk kurasi momen viral dan batch rendering video vertikal 9:16.")

    # Left Column: Embed Real AI Automation Dashboard Screenshot
    left_ai_card = add_card(slide7, Inches(0.8), Inches(1.85), Inches(6.8), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    ai_img = os.path.abspath("assets/images/project-ai-automation.png")
    if os.path.exists(ai_img):
        slide7.shapes.add_picture(ai_img, Inches(0.95), Inches(2.0), Inches(6.5), Inches(4.45))
        tb_ai_cap = slide7.shapes.add_textbox(Inches(0.95), Inches(6.52), Inches(6.5), Inches(0.38))
        tf_cap = tb_ai_cap.text_frame
        tf_cap.margin_left = tf_cap.margin_top = 0
        add_p(tf_cap, "Production Dashboard: Double Coverage Campaign Auto-Clipper (9:16 Vertical Processing & Hook Scoring).", font_size=8.5, italic=True, color=TEXT_MUTED)

    # Right Column: Metrics & Architecture Storytelling
    right_ai_card = add_card(slide7, Inches(7.8), Inches(1.85), Inches(4.7), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    
    # 3 Metric KPI Cards inside right card
    ai_kpis = [
        ("~70%", "REDUKSI TURNAROUND", "Penyusutan waktu dari transkrip mentah ke video"),
        ("10/10", "VIRAL HOOK SCORING", "Deteksi psikologis emosi, shock & virality rating"),
        ("100%", "9:16 BATCH RENDERING", "Watermark auto-applied, dynamic subtitles & BGM")
    ]
    for idx, (val, title, sub) in enumerate(ai_kpis):
        k_top = Inches(2.0 + idx * 0.95)
        k_card = add_card(slide7, Inches(8.0), k_top, Inches(4.3), Inches(0.82), PILL_BG_BLUE, BORDER_ACCENT, radius=0.04)
        tb_k = slide7.shapes.add_textbox(Inches(8.15), k_top + Inches(0.08), Inches(4.0), Inches(0.68))
        tf_k = tb_k.text_frame
        tf_k.margin_left = tf_k.margin_top = 0
        
        p_val = tf_k.paragraphs[0]
        p_val.text = f"{val}  "
        p_val.font.name = FONT_FAMILY
        p_val.font.size = Pt(16)
        p_val.font.bold = True
        p_val.font.color.rgb = NAVY_PRIMARY
        for r_k in p_val.runs:
            r_k.font.name = FONT_FAMILY
            r_k.font.size = Pt(16)
            r_k.font.bold = True
            r_k.font.color.rgb = NAVY_PRIMARY

        r_t = p_val.add_run()
        r_t.text = f"— {title}"
        r_t.font.name = FONT_FAMILY
        r_t.font.size = Pt(9.5)
        r_t.font.bold = True
        r_t.font.color.rgb = BLUE_ACCENT
        
        add_p(tf_k, sub, font_size=8.5, color=TEXT_MUTED)

    # Insights Text Box below KPIs
    tb_ai_ins = slide7.shapes.add_textbox(Inches(8.0), Inches(5.0), Inches(4.3), Inches(1.9))
    tf_ins = tb_ai_ins.text_frame
    tf_ins.word_wrap = True
    tf_ins.margin_left = tf_ins.margin_top = 0

    add_p(tf_ins, "FITUR & ARSITEKTUR MULTI-AGEN:", font_size=10, bold=True, color=NAVY_PRIMARY, space_after=Pt(3))

    ai_features = [
        "Curated Viral Moments: Deteksi otomatis hook 3 detik pembuka, segmentasi timestamp akurat, dan scoring virality 10/10.",
        "Programmatic 9:16 Center Fill: Rendering batch otomatis via script CapCut & engine FFmpeg dengan auto-framing pembicara.",
        "Whop Content Rewards Ready: Dilengkapi 30-min tracker checklist, pemilihan BGM ambient viral, dan vault klip siap publish."
    ]
    for ft in ai_features:
        add_p(tf_ins, f"• {ft}", font_size=8.5, color=TEXT_MAIN, space_after=Pt(3))

    # =========================================================================
    # SLIDE 8: PROJECT 4 - AMAZON SALES & PROFITABILITY DASHBOARD (POLISHED)
    # =========================================================================
    slide8 = prs.slides.add_slide(blank_layout)
    set_background(slide8, BG_LIGHT)
    add_header(slide8, "PROJECT 04: COMMERCIAL ANALYTICS", "Amazon Commercial Sales & Regional Profitability", "Eksplorasi data transaksi e-commerce multi-regional untuk optimalisasi marjin dan logistik.")

    analysis_cards = [
        {
            "tag": "ANALISIS 01",
            "title": "Profitabilitas Regional & Logistik",
            "points": [
                "Pemetaan pendapatan kotor vs net margin di seluruh regional pasar komersial.",
                "Mengidentifikasi anomali di mana volume transaksi tinggi pada state tertentu mengalami erosi profit akibat tingginya biaya ongkos kirim (shipping freight).",
                "Rekomendasi: Penyesuaian threshold minimum basket size untuk gratis ongkir di zona logistik jauh."
            ],
            "stat": "+4.2% Potensi Pemulihan Margin"
        },
        {
            "tag": "ANALISIS 02",
            "title": "Kategori Produk & Kontribusi Marjin",
            "points": [
                "Penerapan Analisis Pareto (80/20) pada kategori produk (Technology, Office Supplies, Furniture).",
                "Memisahkan produk kategori 'High-Volume High-Margin' dari produk 'Cash Trap' yang over-diskon namun menghasilkan profit negatif.",
                "Rekomendasi: Re-alokasi anggaran promosi ke sub-kategori bernilai tambah tinggi."
            ],
            "stat": "80/20 Prinsip Optimalisasi Diskon"
        },
        {
            "tag": "ANALISIS 03",
            "title": "Segmentasi Pelanggan (B2B vs B2C)",
            "points": [
                "Eksplorasi perilaku pembelian antara segmen Korporasi, Home Office, dan Konsumen Langsung.",
                "Menemukan segmen Korporat memiliki repeat order rate 38% lebih konsisten dengan margin stabil.",
                "Rekomendasi: Perancangan skema dedicated account support dan tiering harga B2B berbasis volume."
            ],
            "stat": "38% Repeat Order Lebih Tinggi di B2B"
        }
    ]

    for idx, card_data in enumerate(analysis_cards):
        c_left = Inches(0.8 + idx * 4.0)
        c_top = Inches(1.85)
        c_w = Inches(3.7)
        c_h = Inches(3.65)

        card = add_card(slide8, c_left, c_top, c_w, c_h, BG_CARD, BORDER_SUBTLE, radius=0.02)
        tb = slide8.shapes.add_textbox(c_left + Inches(0.2), c_top + Inches(0.18), c_w - Inches(0.4), c_h - Inches(0.3))
        tf = tb.text_frame
        tf.word_wrap = True
        tf.margin_left = tf.margin_top = 0

        add_p(tf, card_data["tag"], font_size=9, bold=True, color=BLUE_ACCENT, space_after=Pt(1))
        add_p(tf, card_data["title"], font_size=12, bold=True, color=NAVY_PRIMARY, space_after=Pt(4))

        for pt in card_data["points"]:
            add_p(tf, f"• {pt}", font_size=8.5, color=TEXT_MAIN, space_after=Pt(3))

        add_p(tf, f"★ {card_data['stat']}", font_size=9, bold=True, color=NAVY_ACCENT, space_before=Pt(6))

    val_card = add_card(slide8, Inches(0.8), Inches(5.65), Inches(11.7), Inches(1.35), PILL_BG_BLUE, BORDER_ACCENT, radius=0.015)
    tb_val = slide8.shapes.add_textbox(Inches(1.05), Inches(5.75), Inches(11.2), Inches(1.15))
    tf_val = tb_val.text_frame
    tf_val.word_wrap = True
    tf_val.margin_left = tf_val.margin_top = 0

    add_p(tf_val, "DAMPAK STRATEGIS BISNIS YANG DIHASILKAN:", font_size=10.5, bold=True, color=NAVY_PRIMARY, space_after=Pt(2))
    add_p(tf_val, "Dashboard Tableau ini mentransformasi ratusan ribu data transaksi mentah menjadi insight aksi komersial nyata. Manajemen dapat langsung mendeteksi kebocoran margin, memangkas diskon kontraproduktif, serta mengoptimalkan jalur pengiriman antar-negara bagian untuk memulihkan profitabilitas e-commerce secara terukur.", font_size=9.5, color=TEXT_MUTED)

    # =========================================================================
    # SLIDE 9: PENDIDIKAN, PRESTASI & SERTIFIKASI PROFESIONAL (POLISHED)
    # =========================================================================
    slide9 = prs.slides.add_slide(blank_layout)
    set_background(slide9, BG_LIGHT)
    add_header(slide9, "CREDENTIALS & HONORS", "Pendidikan, Prestasi Akademik & Sertifikasi", "Fondasi akademis kuat berpredikat Magna Cum Laude dipadukan dengan sertifikasi industri.")

    # Left Card: Education & Honors
    edu_card = add_card(slide9, Inches(0.8), Inches(1.85), Inches(5.7), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    edu_strip = slide9.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(1.85), Inches(5.7), Inches(0.08))
    edu_strip.fill.solid()
    edu_strip.fill.fore_color.rgb = BLUE_ACCENT
    edu_strip.line.fill.background()

    tb_edu = slide9.shapes.add_textbox(Inches(1.05), Inches(2.05), Inches(5.2), Inches(4.8))
    tf_edu = tb_edu.text_frame
    tf_edu.word_wrap = True
    tf_edu.margin_left = tf_edu.margin_top = 0

    add_p(tf_edu, "PENDIDIKAN FORMAL", font_size=10, bold=True, color=BLUE_ACCENT, space_after=Pt(2))
    add_p(tf_edu, "Universitas Budi Luhur Jakarta", font_size=16, bold=True, color=NAVY_PRIMARY, space_after=Pt(1))
    add_p(tf_edu, "Sarjana Sistem Informasi (S.Kom.) — Peminatan Data Science\nPeriode Studi: 2022 – 2026", font_size=10.5, color=TEXT_MUTED, space_after=Pt(6))

    # Magna Cum Laude pill inside left card
    honor_box = slide9.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.05), Inches(3.2), Inches(5.2), Inches(0.42))
    honor_box.fill.solid()
    honor_box.fill.fore_color.rgb = GOLD_BG
    honor_box.line.color.rgb = GOLD_BORDER
    honor_box.line.width = Pt(1)
    if hasattr(honor_box, "adjustments") and len(honor_box.adjustments) > 0:
        honor_box.adjustments[0] = 0.15
    tf_hb = honor_box.text_frame
    tf_hb.margin_left = Inches(0.15)
    tf_hb.margin_top = Inches(0.06)
    add_p(tf_hb, "★ PREDIKAT: MAGNA CUM LAUDE (IPK 3.88 / 4.00)", font_size=11, bold=True, color=GOLD_TEXT)

    tb_edu_bot = slide9.shapes.add_textbox(Inches(1.05), Inches(3.8), Inches(5.2), Inches(3.0))
    tf_eb = tb_edu_bot.text_frame
    tf_eb.word_wrap = True
    tf_eb.margin_left = tf_eb.margin_top = 0

    edu_details = [
        "Fokus Kurikulum: Algoritma & Pemrograman, Database Relasional, Data Mining, Machine Learning, Statistika Komputasi, Business Intelligence.",
        "Publikasi Skripsi: Komparasi SVM, Naive Bayes dan KNN dalam Analisis Sentimen Respons Netizen YouTube Kasus Korupsi (Nilai Sidang: A).",
        "Riset Terapan: Pengembangan arsitektur NLP presisi tinggi dan dashboard analitika bisnis interaktif."
    ]
    for pt in edu_details:
        add_p(tf_eb, f"• {pt}", font_size=9, color=TEXT_MAIN, space_after=Pt(3))

    add_p(tf_eb, "\nMATA KULIAH UNGGULAN:", font_size=9.5, bold=True, color=NAVY_PRIMARY, space_after=Pt(3))
    coursework = "Data Mining • Machine Learning • Database Systems • Business Intelligence • Statistika Terapan • Metodologi Penelitian Data"
    add_p(tf_eb, coursework, font_size=8.5, italic=True, color=NAVY_ACCENT, space_after=Pt(2))

    # Right Card: Professional Certifications & Continuous Upskilling
    cert_card = add_card(slide9, Inches(6.8), Inches(1.85), Inches(5.7), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    cert_strip = slide9.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(6.8), Inches(1.85), Inches(5.7), Inches(0.08))
    cert_strip.fill.solid()
    cert_strip.fill.fore_color.rgb = CYAN_ACCENT
    cert_strip.line.fill.background()

    tb_cert = slide9.shapes.add_textbox(Inches(7.05), Inches(2.05), Inches(5.2), Inches(4.8))
    tf_cert = tb_cert.text_frame
    tf_cert.word_wrap = True
    tf_cert.margin_left = tf_cert.margin_top = 0

    add_p(tf_cert, "SERTIFIKASI & KOMPETENSI INDUSTRI", font_size=10, bold=True, color=BLUE_ACCENT, space_after=Pt(4))

    certs = [
        ("RevoU Data Analytics Specialization", "Business Analytics, SQL Querying, Tableau Storytelling, and Data-Driven Strategy formulation.", "Credential Verified"),
        ("DQLab: Python for Data Science & Viz", "Data wrangling with Pandas/NumPy, exploratory data analysis, and statistical visualization.", "Verified Certificate"),
        ("Google Antigravity Agentic AI Mastery", "Autonomous multi-agent orchestration, LLM prompt engineering, and end-to-end automated pipelines.", "Industry Applied"),
        ("Database Relasional & SQL Mastery", "Skema relational database design, CTEs, Window Functions, and query optimization.", "Hands-on Project")
    ]

    for c_name, c_desc, c_tag in certs:
        p_c = tf_cert.add_paragraph()
        p_c.text = f"✓ {c_name} "
        p_c.font.name = FONT_FAMILY
        p_c.font.size = Pt(10)
        p_c.font.bold = True
        p_c.font.color.rgb = NAVY_PRIMARY
        p_c.space_after = Pt(1)
        for r_c in p_c.runs:
            r_c.font.name = FONT_FAMILY
            r_c.font.size = Pt(10)
            r_c.font.bold = True
            r_c.font.color.rgb = NAVY_PRIMARY

        r_tag = p_c.add_run()
        r_tag.text = f"[{c_tag}]"
        r_tag.font.name = FONT_FAMILY
        r_tag.font.size = Pt(8.5)
        r_tag.font.bold = True
        r_tag.font.color.rgb = EMERALD_TEXT

        add_p(tf_cert, c_desc, font_size=8.5, color=TEXT_MUTED, space_after=Pt(6))

    # =========================================================================
    # SLIDE 10: NILAI TAMBAH, TARGET PERAN & KONTAK PROFESIONAL (POLISHED)
    # =========================================================================
    slide10 = prs.slides.add_slide(blank_layout)
    set_background(slide10, BG_LIGHT)
    add_header(slide10, "LET'S COLLABORATE", "Nilai Tambah, Target Peran & Kontak Profesional", "Siap berkontribusi langsung memberikan dampak bisnis nyata melalui analitika data dan otomasi AI.")

    # Left Section: Value Proposition & Target Roles
    left_col_card = add_card(slide10, Inches(0.8), Inches(1.85), Inches(6.0), Inches(5.15), BG_CARD, BORDER_SUBTLE, radius=0.02)
    tb_vp = slide10.shapes.add_textbox(Inches(1.05), Inches(2.05), Inches(5.5), Inches(4.8))
    tf_vp = tb_vp.text_frame
    tf_vp.word_wrap = True
    tf_vp.margin_left = tf_vp.margin_top = 0

    add_p(tf_vp, "APA YANG SAYA HADIRKAN BAGI TIM ANDA?", font_size=10.5, bold=True, color=BLUE_ACCENT, space_after=Pt(4))

    vp_items = [
        ("Kombinasi Teknis & Bisnis", "Tidak hanya mengolah angka, tetapi mampu mengartikulasikan insight analitika menjadi rekomendasi keputusan strategis bagi manajemen."),
        ("Riset Machine Learning Teruji", "Terbukti mampu merancang pipeline ML NLP akurasi tinggi (88.4%) dengan metodologi evaluasi yang valid dan bebas bias."),
        ("Adopsi Teknologi Agentic AI Terdepan", "Membawa kapabilitas otomatisasi masa depan untuk menghemat biaya operasional dan mempercepat time-to-delivery proyek.")
    ]

    for v_title, v_desc in vp_items:
        p_v = tf_vp.add_paragraph()
        p_v.text = f"❖ {v_title}: "
        p_v.font.name = FONT_FAMILY
        p_v.font.size = Pt(9.5)
        p_v.font.bold = True
        p_v.font.color.rgb = NAVY_PRIMARY
        p_v.space_after = Pt(2)
        for r_v in p_v.runs:
            r_v.font.name = FONT_FAMILY
            r_v.font.size = Pt(9.5)
            r_v.font.bold = True
            r_v.font.color.rgb = NAVY_PRIMARY
        r = p_v.add_run()
        r.text = v_desc
        r.font.name = FONT_FAMILY
        r.font.size = Pt(8.5)
        r.font.bold = False
        r.font.color.rgb = TEXT_MUTED

    add_p(tf_vp, "TARGET PERAN & KESIAPAN KERJA:", font_size=10, bold=True, color=NAVY_PRIMARY, space_before=Pt(6), space_after=Pt(3))

    roles = [
        ("Data Analyst", "EDA, SQL complex queries, Tableau reporting, KPI metrics tracking."),
        ("Business Intelligence Analyst", "Dashboard architecture, data storytelling, executive insights."),
        ("Junior Data Scientist", "Predictive modeling, NLP pipelines, Scikit-Learn, model deployment."),
        ("Agentic AI Specialist", "Workflow orchestration, multi-agent frameworks, task automation.")
    ]
    for r_title, r_desc in roles:
        p_r = tf_vp.add_paragraph()
        p_r.text = f"• {r_title}: "
        p_r.font.name = FONT_FAMILY
        p_r.font.size = Pt(9)
        p_r.font.bold = True
        p_r.font.color.rgb = BLUE_ACCENT
        p_r.space_after = Pt(1)
        for r_rl in p_r.runs:
            r_rl.font.name = FONT_FAMILY
            r_rl.font.size = Pt(9)
            r_rl.font.bold = True
            r_rl.font.color.rgb = BLUE_ACCENT
        r_sub = p_r.add_run()
        r_sub.text = r_desc
        r_sub.font.name = FONT_FAMILY
        r_sub.font.size = Pt(8.5)
        r_sub.font.bold = False
        r_sub.font.color.rgb = TEXT_MUTED

    # Right Section: Direct Contact Cards
    right_contact_card = add_card(slide10, Inches(7.1), Inches(1.85), Inches(5.4), Inches(5.15), NAVY_PRIMARY, border_color=None, radius=0.02)
    tb_rc = slide10.shapes.add_textbox(Inches(7.35), Inches(2.05), Inches(4.9), Inches(4.8))
    tf_rc = tb_rc.text_frame
    tf_rc.word_wrap = True
    tf_rc.margin_left = tf_rc.margin_top = 0

    add_p(tf_rc, "HUBUNGI SAYA LANGSUNG", font_size=12, bold=True, color=CYAN_ACCENT, space_after=Pt(1))
    add_p(tf_rc, "Terbuka untuk peluang kerja Full-time, Kontrak, atau Kolaborasi Proyek Strategis.", font_size=8.5, color=RGBColor(226, 232, 240), space_after=Pt(4))

    # Availability pill
    avail_box = slide10.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7.35), Inches(2.68), Inches(4.9), Inches(0.35))
    avail_box.fill.solid()
    avail_box.fill.fore_color.rgb = RGBColor(6, 78, 59) # Dark Emerald
    avail_box.line.color.rgb = RGBColor(16, 185, 129)
    avail_box.line.width = Pt(1)
    if hasattr(avail_box, "adjustments") and len(avail_box.adjustments) > 0:
        avail_box.adjustments[0] = 0.2
    tf_av = avail_box.text_frame
    tf_av.margin_left = Inches(0.12)
    tf_av.margin_top = Inches(0.04)
    add_p(tf_av, "● TERSEDIA SEGERA (IMMEDIATE AVAILABILITY) — JAKARTA / HYBRID / REMOTE", font_size=8.5, bold=True, color=RGBColor(167, 243, 208))

    tb_contact_details = slide10.shapes.add_textbox(Inches(7.35), Inches(3.12), Inches(4.9), Inches(3.7))
    tf_cd = tb_contact_details.text_frame
    tf_cd.word_wrap = True
    tf_cd.margin_left = tf_cd.margin_top = 0

    contact_items = [
        ("EMAIL UTAMA", "khairulraihan1204@gmail.com"),
        ("WHATSAPP / SELULER", "+62 822-1081-3652"),
        ("PROFIL LINKEDIN", "linkedin.com/in/khairulraihan"),
        ("REPOSITORI GITHUB", "github.com/KhairulRaihan"),
        ("WEB PORTOFOLIO", "khairulraihan.github.io"),
        ("BAHASA & LOKASI", "Indonesia (Native), English (Professional) | Jakarta")
    ]

    for lbl, val in contact_items:
        add_p(tf_cd, lbl, font_size=7.5, bold=True, color=CYAN_ACCENT, space_after=Pt(0))
        add_p(tf_cd, val, font_size=9, color=TEXT_WHITE, space_after=Pt(4))

    # Save to all target files
    out_files = [
        "Biru Putih Minimalis Modern Profesional Portofolio Presentasi.pptx",
        "Khairul_Raihan_Hidayat_Portfolio_Deck.pptx",
        "assets/docs/Khairul_Raihan_Hidayat_Portfolio_Deck.pptx"
    ]

    for fpath in out_files:
        os.makedirs(os.path.dirname(fpath) if os.path.dirname(fpath) else ".", exist_ok=True)
        prs.save(fpath)
        print(f"Successfully generated: {fpath}")

if __name__ == "__main__":
    build_polished_presentation()
